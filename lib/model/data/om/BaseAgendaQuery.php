 ...
 * @method AgendaQuery orderByFechaControl($order = Criteria::ASC) Order by the fecha_control column
 ...
 * @method AgendaQuery groupByFechaControl() Group by the fecha_control column
 ...
 * @method Agenda findOneByFechaControl(string $fecha_control) Return the first Agenda filtered by the fecha_control column
 ...
 * @method array findByFechaControl(string $fecha_control) Return Agenda objects filtered by the fecha_control column
...
abstract class BaseAgendaQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query on the fecha_control column
     *
     * Example usage:
     * <code>
     * $query->filterByFechaControl('2011-03-14'); // WHERE fecha_control = '2011-03-14'
     * $query->filterByFechaControl('now'); // WHERE fecha_control = '2011-03-14'
     * $query->filterByFechaControl(array('max' => 'yesterday')); // WHERE fecha_control > '2011-03-13'
     * </code>
     *
     * @param     mixed $fechaControl The value to use as filter.
     *              Values can be integers (unix timestamps), DateTime objects, or strings.
     *              Empty strings are treated as NULL.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     ...
     * @return AgendaQuery The current query, for fluid interface
     */
    public function filterByFechaControl($fechaControl = null, $comparison = null)
    {
        if (is_array($fechaControl)) {
            $useMinMax = false;
            if (isset($fechaControl['min'])) {
                $this->addUsingAlias(AgendaPeer::FECHA_CONTROL, $fechaControl['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($fechaControl['max'])) {
                $this->addUsingAlias(AgendaPeer::FECHA_CONTROL, $fechaControl['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AgendaPeer::FECHA_CONTROL, $fechaControl, $comparison);
    }
