...
/**
 * Base class that represents a row from the 'hc_usuarios' table.
 *
 *
 *
...
abstract class BaseUser extends BaseObject implements Persistent
{
    ...
    /**
     * The unserialized $tipo value - i.e. the persisted object.
     * This is necessary to avoid repeated calls to unserialize() at runtime.
     * @var        object
     */
    protected $tipo_unserialized;
    ...
    {
        if (null === $this->tipo_unserialized) {
            $this->tipo_unserialized = array();
        }
        if (!$this->tipo_unserialized && null !== $this->tipo) {
            $tipo_unserialized = substr($this->tipo, 2, -2);
            $this->tipo_unserialized = $tipo_unserialized ? explode(' | ', $tipo_unserialized) : array();
        }

        return $this->tipo_unserialized;
    }
    ...
    public function setNombre($v)
    {
        ...
        if ($this->nombre !== $v) {
            $this->nombre = $v;
            $this->modifiedColumns[] = UserPeer::NOMBRE;
        }
    ...
    {
        if ($this->tipo_unserialized !== $v) {
            $this->tipo_unserialized = $v;
            $this->tipo = '| ' . implode(' | ', $v) . ' |';
            $this->modifiedColumns[] = UserPeer::TIPO;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->tipo = $row[$startcol + 2];
            $this->tipo_unserialized = null;
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UserPeer::NOMBRE)) {
            $modifiedColumns[':p' . $index++]  = '`NOMBRE`';
        }
        if ($this->isColumnModified(UserPeer::PASS)) {
            $modifiedColumns[':p' . $index++]  = '`PASS`';
        }
        if ($this->isColumnModified(UserPeer::TIPO)) {
        ...
        $sql = sprintf(
            'INSERT INTO `hc_usuarios` (%s) VALUES (%s)',
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserPeer::NOMBRE)) $criteria->add(UserPeer::NOMBRE, $this->nombre);
        if ($this->isColumnModified(UserPeer::PASS)) $criteria->add(UserPeer::PASS, $this->pass);
        if ($this->isColumnModified(UserPeer::TIPO)) $criteria->add(UserPeer::TIPO, $this->tipo);
